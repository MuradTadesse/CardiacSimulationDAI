...
void cmdLine(int argc, char *argv[], double& T, int& n, int& px, int& py, int& plot_freq, int& no_comm, int& num_threads) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "-x")) px = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-y")) py = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-p")) plot_freq = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-c")) no_comm = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-nt")) num_threads = atoi(argv[++i]);
    }
}
...
void simulate (double** E,  double** E_prev,double** R,
		...
		const double M1,const double  M2, const double b,int n,int m, int px, int py, int num_threads){
	/*******************************************************************/
	double t = 0.0;
	int root = 0;
	int niter;
	...
	// }
// In the simulate function, replace the existing communication code with:

	// For vertical (py > 1) communication
	if (py > 1) {
		MPI_Request requests[4];
		MPI_Status statuses[4];
		int req_cnt = 0;

		if (rowid == 0) { // Top process
			MPI_Isend(&E_prev[m][1], n, MPI_DOUBLE, rank + px, NB, MPI_COMM_WORLD, &requests[req_cnt++]);
			MPI_Irecv(&E_prev[m+1][1], n, MPI_DOUBLE, rank + px, NB, MPI_COMM_WORLD, &requests[req_cnt++]);
		}
		else if (rowid == py-1) { // Bottom process
			MPI_Isend(&E_prev[1][1], n, MPI_DOUBLE, rank - px, NB, MPI_COMM_WORLD, &requests[req_cnt++]);
			MPI_Irecv(&E_prev[0][1], n, MPI_DOUBLE, rank - px, NB, MPI_COMM_WORLD, &requests[req_cnt++]);
		}
		...
		MPI_Waitall(req_cnt, requests, statuses);
	}
		// For horizontal (px > 1) communication
	if (px > 1) {
		MPI_Request requests[4];
		MPI_Status statuses[4];
		int req_cnt = 0;
		...
		for (int j=1; j<=m; j++) {
			sendbuffer[j-1] = E_prev[j][n];    // Right boundary
			sendbuffer2[j-1] = E_prev[j][1];   // Left boundary
		}

		if (colid == 0) { // Leftmost process
			MPI_Isend(sendbuffer, m, MPI_DOUBLE, rank+1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
			MPI_Irecv(recvbuffer, m, MPI_DOUBLE, rank+1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
		}
		else if (colid == px-1) { // Rightmost process
			MPI_Isend(sendbuffer2, m, MPI_DOUBLE, rank-1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
			MPI_Irecv(recvbuffer2, m, MPI_DOUBLE, rank-1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
		}
		else { // Middle processes
			MPI_Isend(sendbuffer, m, MPI_DOUBLE, rank+1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
			MPI_Irecv(recvbuffer, m, MPI_DOUBLE, rank+1, NC_B, MPI_COMM_WORLD, &requests[req_cnt++]);
		...
		#pragma omp parallel for num_threads(num_threads)
		for (int j=1; j<=m; j++) {
			if (colid == 0) E_prev[j][n+1] = recvbuffer[j-1];
			else if (colid == px-1) E_prev[j][0] = recvbuffer2[j-1];
			else {
				E_prev[j][n+1] = recvbuffer[j-1];
				E_prev[j][0] = recvbuffer2[j-1];
			}
	...
	// Original code with non-contiguous memory access
	for (j=1; j<=m; j++) {
		for (i=1; i<=n; i++) {
	...
	// // Optimized version with better spatial locality
...
int main (int argc, char** argv)
{
	...
	while (t<T) {
		...
		simulate(E, E_prev, R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b,col,row, px,py, num_threads);
		...
		if (plot_freq) {
			...
			if ((t - k * plot_freq) < dt) {
				// Gather data to rank 0 before plotting
				if (myrank == 0) {
					// Only plot if we have valid data
					if (E_prev != NULL && m > 0 && n > 0) {
						splot(E_prev, t, niter, row+2, col+2);
					}
				}
			}
			// Make sure all processes wait for plotting to complete
